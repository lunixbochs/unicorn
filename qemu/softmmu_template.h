                            ...
                            uintptr_t retaddr)
{
    ...
    if (mr == NULL) {
        ...
        HOOK_FOREACH_RANGE(uc, hook, UC_HOOK_MEM_FETCH_UNMAPPED, addr) {
        ...
        HOOK_FOREACH_RANGE(uc, hook, UC_HOOK_MEM_READ_UNMAPPED, addr) {
    ...
    if (mr != NULL && !(mr->perms & UC_PROT_EXEC)) {  // non-executable
        handled = 0;
        HOOK_FOREACH_RANGE(uc, hook, UC_HOOK_MEM_FETCH_PROT, addr) {
    ...
    if (READ_ACCESS_TYPE == MMU_DATA_LOAD) {
        HOOK_FOREACH_RANGE(uc, hook, UC_HOOK_MEM_READ, addr) {
    ...
    if (READ_ACCESS_TYPE == MMU_DATA_LOAD && mr != NULL && !(mr->perms & UC_PROT_READ)) {  //non-readable
        handled = 0;
        HOOK_FOREACH_RANGE(uc, hook, UC_HOOK_MEM_READ_PROT, addr) {
                       ...
                       int mmu_idx, uintptr_t retaddr)
{
    ...
    HOOK_FOREACH_RANGE(uc, hook, UC_HOOK_MEM_WRITE, addr) {
    ...
    if (mr == NULL) {
        handled = 0;
        HOOK_FOREACH_RANGE(uc, hook, UC_HOOK_MEM_WRITE_UNMAPPED, addr) {
    ...
    if (mr != NULL && !(mr->perms & UC_PROT_WRITE)) {  //non-writable
        handled = 0;
        HOOK_FOREACH_RANGE(uc, hook, UC_HOOK_MEM_WRITE_PROT, addr) {
