...
// for loop macros to loop over hook lists
#define HOOK_FOREACH_COMMON(uc, hh, idx, extra)                            \
    struct list_item *cur;                                                 \
    for (                                                                  \
    cur = (uc)->hook[idx##_IDX].head;                                      \
    cur != NULL && ((hh) = (struct hook *)cur->data)                       \
        && !uc->stop_request /* stop excuting callbacks on stop request */ \
        extra;               /* extra is used below for address ranges */  \
    cur = cur->next)

// loop without address check
#define HOOK_FOREACH(uc, hh, idx) HOOK_FOREACH_COMMON(uc, hh, idx, )

// loop filtering against begin/end address
#define HOOK_FOREACH_RANGE(uc, hh, idx, addr)  \
    HOOK_FOREACH_COMMON(uc, hh, idx,           \
        && (((addr) >= hook->begin && (addr) < hook->end) || hook->begin > hook->end))
